...
async function train() {
  ...
  await model.fit(xs, ys, {
    ...
    epochs: 100,
    callbacks: {
      onTrainStart: console.log('Training started'),
...
function draw() {
  ...
  // Tidy method clears the tensor memory from the WEBGL, it is garbage collector
  tf.tidy(() => {
    const input = tf.tensor2d([
      [r, g, b]
    ]);
    let results = model.predict(input);
    let argMax = results.argMax(1); // This method finds the biggest range from the result tensor array
